...
app.get('/api/vehiculos', async (req, res) => {
  try {
    ...
    const [rows] = await connection.execute(`
      ...
      LEFT JOIN tbl_clientes c ON v.fk_id_cliente = c.PK_id_cliente
...
app.get('/api/ordenes', async (req, res) => {
  try {
    const connection = await mysql.createConnection(dbConfig);
    const [rows] = await connection.execute(`
      SELECT o.*, c.nombre_cliente, c.apellido_cliente, v.placa_vehiculo, v.marca_vehiculo, v.modelo_vehiculo, s.servicio
      FROM tbl_ordenes o
